globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/link-CzjjZDHK.js","assets/navigation-C71DNc6T.js","assets/admin-app-BpLgQ_F6.js","assets/storefront-Qyt95yzr.js","assets/page-44B7ub3R.js","assets/page-DOpZI6eJ.js","assets/layout-segment-context-DOvbdA9A.js","assets/router-CK9cLTOB.js"];
...
function getDraftSecret() {
	return "61e19a7c-0ae8-460d-9bbd-788fa87d7278";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "59a7d256-1077-4c65-9a4c-928d2e197a52";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "59a7d256-1077-4c65-9a4c-928d2e197a52") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "59a7d256-1077-4c65-9a4c-928d2e197a52",
		rootBoundaryId
	});
}
/**
...
async function ensureSchema() {
	...
	schemaReady = (async () => {
		...
		await d1.batch([d1.prepare(`INSERT OR IGNORE INTO menus
        (id, name, description, price, image_url, image_position, is_available, sort_order, created_at, updated_at)
        VALUES (?, ?, ?, ?, NULL, 'left center', 1, 1, ?, ?)`).bind("boiled-chicken", "ข้าวมันไก่ต้ม", "ไก่นุ่มฉ่ำ ข้าวหอม น้ำจิ้มเต้าเจี้ยวสูตรบ้านเรา", 55, now, now), d1.prepare(`INSERT OR IGNORE INTO menus
        (id, name, description, price, image_url, image_position, is_available, sort_order, created_at, updated_at)
        VALUES (?, ?, ?, ?, NULL, 'right center', 1, 2, ?, ?)`).bind("fried-chicken", "ข้าวมันไก่ทอด", "ไก่ทอดกรอบนอกนุ่มใน เสิร์ฟคู่ซอสหวานเผ็ด", 60, now, now)]);
	})().catch((error) => {
		schemaReady = null;
		throw error;
	});
...
function bangkokDate(date = /* @__PURE__ */ new Date()) {
...
async function readJson(request) {
	if (!(request.headers.get("content-type") ?? "").includes("application/json")) throw new Error("รูปแบบข้อมูลไม่ถูกต้อง");
...
async function makeAdminSession(request) {
	const { secret } = authConfig(request);
	if (!secret) throw new Error("ยังไม่ได้ตั้งค่าความปลอดภัยของแอดมิน");
	const value = `${Math.floor(Date.now() / 1e3) + 3600 * 12}.admin`;
	return `${value}.${await hmac(value, secret)}`;
}
async function isAdmin(request) {
	const { secret } = authConfig(request);
	if (!secret) return false;
	const [expiresText, role, signature] = cookieValue(request, "admin_session").split(".");
	const expires = Number(expiresText);
	if (!expires || role !== "admin" || expires < Date.now() / 1e3 || !signature) return false;
	return safeEqual(signature, await hmac(`${expiresText}.${role}`, secret));
}
async function requireAdmin(request) {
	if (!await isAdmin(request)) throw new AdminAuthError();
}
var AdminAuthError = class extends Error {
	constructor() {
...
async function audit(action, entityType, entityId, detail = "") {
	await db().prepare("INSERT INTO audit_logs (action, entity_type, entity_id, detail, created_at) VALUES (?, ?, ?, ?, ?)").bind(action, entityType, entityId, detail, (/* @__PURE__ */ new Date()).toISOString()).run();
}
//#endregion
//#region app/api/admin/orders/[id]/route.ts
var route_exports$11 = /* @__PURE__ */ __exportAll({ PATCH: () => PATCH$1 });
var allowed = [
	"received",
	"cooking",
	"ready",
...
async function PATCH$1(request, context) {
	try {
		...
		if (!payload.status || !allowed.includes(payload.status)) return Response.json({ error: "สถานะไม่ถูกต้อง" }, { status: 400 });
		const now = (/* @__PURE__ */ new Date()).toISOString();
		if (!(await db().prepare("UPDATE orders SET status = ?, updated_at = ?, delivered_at = ? WHERE id = ?").bind(payload.status, now, payload.status === "delivered" ? now : null, id).run()).meta.changes) return Response.json({ error: "ไม่พบออเดอร์" }, { status: 404 });
	...
	return {
		...
		openGraph: {
			...
			images: [{
				...
				width: 1734,
				height: 909,
				alt: "ไก่เฮงเฮง ข้าวมันไก่ต้มและไก่ทอด"
			}]
		},
		twitter: {
...
var Storefront = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'Storefront' is called on server");
}, "f1da2b17eb86", "Storefront");
//#endregion
//#region app/page.tsx
...
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(Storefront, {});
}
//#endregion
//#region app/admin/admin-app.tsx
var AdminApp = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'AdminApp' is called on server");
}, "06ce7da525b0", "AdminApp");
//#endregion
...
function AdminPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(AdminApp, {});
}
//#endregion
//#region app/api/admin/login/route.ts
var route_exports$10 = /* @__PURE__ */ __exportAll({ POST: () => POST$4 });
async function POST$4(request) {
	try {
		const payload = await readJson(request);
		const config = authConfig(request);
		if (!config.pin || !config.secret) return Response.json({ error: "กรุณาตั้งค่า ADMIN_PIN และ SESSION_SECRET ก่อนเปิดใช้หลังบ้าน" }, { status: 503 });
		if ((payload.pin ?? "") !== config.pin) return Response.json({ error: "รหัสผ่านไม่ถูกต้อง" }, { status: 401 });
		const token = await makeAdminSession(request);
		return Response.json({
			ok: true,
			demoPin: config.isLocal ? "2468" : void 0
		}, { headers: { "Set-Cookie": `admin_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Strict; Max-Age=43200${config.isLocal ? "" : "; Secure"}` } });
	} catch (error) {
		return serverError(error);
	}
}
...
async function POST$3() {
	return Response.json({ ok: true }, { headers: { "Set-Cookie": "admin_session=; Path=/; HttpOnly; SameSite=Strict; Max-Age=0" } });
}
//#endregion
//#region app/api/admin/orders/route.ts
...
async function GET$5(request) {
	try {
		...
		const orders = (await db().prepare(`SELECT o.id, o.queue_no AS queueNo, o.queue_date AS queueDate, o.customer_name AS customerName,
       ...
       FROM orders o ${where} ORDER BY o.created_at DESC LIMIT 300`).bind(...values).all()).results.map((row) => ({
			...row,
...
var route_exports$7 = /* @__PURE__ */ __exportAll({ GET: () => GET$4 });
async function GET$4(request) {
	const config = authConfig(request);
	return Response.json({
		authenticated: await isAdmin(request),
		localDemo: config.isLocal
	});
}
//#endregion
//#region app/api/admin/summary/route.ts
var route_exports$6 = /* @__PURE__ */ __exportAll({ GET: () => GET$3 });
async function GET$3(request) {
	try {
		...
		const [todaySummary, monthSummary, statusCounts, dailyRows, topMenus] = await Promise.all([
			db().prepare(`SELECT COUNT(*) AS orders, COALESCE(SUM(total), 0) AS sales
         FROM orders WHERE queue_date = ? AND status != 'cancelled'`).bind(today).first(),
			db().prepare(`SELECT COUNT(*) AS orders, COALESCE(SUM(total), 0) AS sales
         FROM orders WHERE substr(queue_date, 1, 7) = ? AND status != 'cancelled'`).bind(month).first(),
			db().prepare(`SELECT status, COUNT(*) AS count FROM orders
         WHERE queue_date = ? GROUP BY status`).bind(today).all(),
			db().prepare(`SELECT queue_date AS date, COUNT(*) AS orders, COALESCE(SUM(total), 0) AS sales
         FROM orders WHERE queue_date >= date(?, '-6 days') AND status != 'cancelled'
         GROUP BY queue_date ORDER BY queue_date`).bind(today).all(),
			db().prepare(`SELECT oi.menu_name AS name, SUM(oi.quantity) AS quantity, SUM(oi.line_total) AS sales
         FROM order_items oi JOIN orders o ON o.id = oi.order_id
         WHERE substr(o.queue_date, 1, 7) = ? AND o.status != 'cancelled'
         GROUP BY oi.menu_name ORDER BY quantity DESC LIMIT 5`).bind(month).all()
		]);
		return Response.json({
			today: todaySummary ?? {
				orders: 0,
...
//#endregion
//#region app/api/admin/upload/route.ts
...
async function POST$2(request) {
	try {
		...
		if (!allowedTypes.has(file.type) || file.size > 5 * 1024 * 1024) return Response.json({ error: "รองรับ JPG, PNG, WebP หรือ AVIF ขนาดไม่เกิน 5 MB" }, { status: 400 });
		const extension = file.type.split("/")[1].replace("jpeg", "jpg");
		const key = `${Date.now()}-${crypto.randomUUID()}.${extension}`;
		await bucket.put(key, await file.arrayBuffer(), { httpMetadata: {
...
async function GET$2() {
	try {
		await ensureSchema();
		const result = await db().prepare(`SELECT id, name, description, price, image_url AS imageUrl,
        image_position AS imagePosition, is_available AS isAvailable, sort_order AS sortOrder
       FROM menus ORDER BY sort_order, created_at`).all();
...
async function POST$1(request) {
	try {
		...
		if (!name || !Number.isFinite(price) || price <= 0 || price > 1e4) return Response.json({ error: "กรุณาระบุชื่อและราคาให้ถูกต้อง" }, { status: 400 });
		const id = crypto.randomUUID();
		const now = (/* @__PURE__ */ new Date()).toISOString();
		await db().prepare(`INSERT INTO menus (id, name, description, price, image_url, image_position, is_available, sort_order, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, name, description, price, payload.imageUrl ?? null, payload.imagePosition ?? "center", payload.isAvailable === false ? 0 : 1, Number(payload.sortOrder) || 99, now, now).run();
...
//#endregion
//#region app/api/orders/route.ts
...
async function POST(request) {
	try {
		...
		const menuResult = await db().prepare(`SELECT id, name, price, is_available FROM menus WHERE id IN (${placeholders})`).bind(...Array.from(quantities.keys())).all();
		if (menuResult.results.length !== quantities.size || menuResult.results.some((menu) => !menu.is_available)) return jsonError("มีเมนูที่ปิดขายหรือไม่พบ กรุณาโหลดหน้าใหม่");
		...
		const counter = await db().prepare(`INSERT INTO queue_counters (queue_date, last_queue_no) VALUES (?, 1)
       ON CONFLICT(queue_date) DO UPDATE SET last_queue_no = last_queue_no + 1
       RETURNING last_queue_no AS queueNo`).bind(queueDate).first();
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'received', ?, ?, ?, ?)`).bind(id, publicToken, queueDate, counter.queueNo, customerName, phone, pickupType, note, subtotal, total, now, now).run();
		await db().batch(items.map((item) => db().prepare(`INSERT INTO order_items (order_id, menu_id, menu_name, unit_price, quantity, line_total)
       VALUES (?, ?, ?, ?, ?, ?)`).bind(id, item.menuId, item.menuName, item.unitPrice, item.quantity, item.lineTotal)));
...
//#endregion
//#region app/api/menu/[id]/route.ts
...
async function PATCH(request, context) {
	try {
		...
		const description = payload.description === void 0 ? String(current.description) : payload.description.trim().slice(0, 240);
		const price = payload.price === void 0 ? Number(current.price) : Number(payload.price);
		if (!name || !Number.isFinite(price) || price <= 0 || price > 1e4) return Response.json({ error: "ข้อมูลเมนูไม่ถูกต้อง" }, { status: 400 });
		await db().prepare(`UPDATE menus SET name = ?, description = ?, price = ?, image_url = ?, image_position = ?,
       is_available = ?, sort_order = ?, updated_at = ? WHERE id = ?`).bind(name, description, price, payload.imageUrl === void 0 ? current.image_url : payload.imageUrl, payload.imagePosition ?? String(current.image_position), payload.isAvailable === void 0 ? Number(current.is_available) : payload.isAvailable ? 1 : 0, payload.sortOrder === void 0 ? Number(current.sort_order) : Number(payload.sortOrder) || 0, (/* @__PURE__ */ new Date()).toISOString(), id).run();
...
//#endregion
//#region app/api/orders/[id]/route.ts
...
async function GET(request, context) {
	try {
		...
		const order = await db().prepare(`SELECT id, queue_no AS queueNo, customer_name AS customerName, pickup_type AS pickupType,
       note, status, total, created_at AS createdAt, updated_at AS updatedAt
       FROM orders WHERE id = ? AND public_token = ?`).bind(id, token).first();
		if (!order) return jsonError("ไม่พบออเดอร์", 404);
		const ahead = order.status === "delivered" || order.status === "cancelled" ? 0 : await db().prepare(`SELECT COUNT(*) AS count FROM orders
         WHERE queue_date = ? AND queue_no < ? AND status NOT IN ('delivered', 'cancelled')`).bind(bangkokDate(new Date(order.createdAt)), order.queueNo).first();
		const itemResult = await db().prepare(`SELECT menu_name AS menuName, unit_price AS unitPrice, quantity, line_total AS lineTotal
       FROM order_items WHERE order_id = ? ORDER BY id`).bind(id).all();
		return Response.json({ order: {
			...order,
			ahead: Number(ahead && "count" in ahead ? ahead.count : 0),
...
var routes = [
	{
		...
		ids: {
			"route": "route:/api/admin/orders/:id",
			"page": null,
			"routeHandler": "route-handler:/api/admin/orders/:id",
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/orders",
			"page": null,
			"routeHandler": "route-handler:/api/admin/orders",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/summary",
			"page": null,
			"routeHandler": "route-handler:/api/admin/summary",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/admin/upload",
			"page": null,
			"routeHandler": "route-handler:/api/admin/upload",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/menu",
			"page": null,
			"routeHandler": "route-handler:/api/menu",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/orders",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/menu/:id",
			"page": null,
			"routeHandler": "route-handler:/api/menu/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/orders/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
